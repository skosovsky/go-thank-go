...
func IsAny(s string, test func(ch rune) bool) bool {
	for _, char := range s {
		if test(char) {
			return true
		}
...
func digits(s string) bool {
	return IsAny(s, unicode.IsDigit)
}
...
func letters(s string) bool {
	return IsAny(s, unicode.IsLetter)
}

// digits возвращает true, если s содержит хотя бы одну цифру
// согласно unicode.IsDigit(), иначе false
func digitsN(s string) bool {
	for _, v := range s {
